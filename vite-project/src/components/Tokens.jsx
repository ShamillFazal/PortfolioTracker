...
function Tokens({ wallet, chain, tokens, setTokens}) {
    ...
    if (response.data) {
        ...
        for (let i = 0; i < tkns.length; i++) {
          tkns[i].bal = (Number(tkns[i].balance) / Number(`1E${tkns[i].decimals}`)).toFixed(3);
          tkns[i].value = (Number(tkns[i].balance) / Number(`1E${tkns[i].decimals}`) * Number(tkns[i].usd)).toFixed(2);
        }
